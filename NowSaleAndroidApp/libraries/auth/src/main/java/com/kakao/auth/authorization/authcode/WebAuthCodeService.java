...
class WebAuthCodeService implements AuthCodeService {
    ...
    public boolean requestAuthCode(AuthCodeRequest request, StartActivityWrapper wrapper, AuthCodeListener listener) {
        try {
            Intent intent;
            if (request.getExtraHeaders().containsKey(StringSet.RT)) {
                intent = createScopeUpdateIntent(wrapper.getContext(), request, listener);
            } else {
                intent = createAccountLoginIntent(wrapper.getContext(), request, listener);
            }
    ...
    Intent createAccountLoginIntent(final Context context, final AuthCodeRequest request, final AuthCodeListener listener) {
        Uri uri = createAccountLoginUri(request);
        return createAuthorizeIntentWithUri(context, request, uri, listener);
    }

    Intent createScopeUpdateIntent(final Context context, final AuthCodeRequest request, final AuthCodeListener listener) {
        Uri uri = createScopeUpdateUri(request);
        return createAuthorizeIntentWithUri(context, request, uri, listener);
    }

    /*
        Create uri for account login page. This page bypasses cookie and
     */
    Uri createAccountLoginUri(final AuthCodeRequest request) {
        Uri continueUri = createScopeUpdateUri(request);
        return new Uri.Builder().scheme(ServerProtocol.SCHEME).authority(ServerProtocol.ACCOUNT_AUTHORITY)
    ...
    Uri createScopeUpdateUri(final AuthCodeRequest request) {
        ...
        return Utility.buildUri(ServerProtocol.AUTH_AUTHORITY, ServerProtocol.AUTHORIZE_CODE_PATH, parameters);
    }
     ...
     */
    Intent createAuthorizeIntentWithUri(final Context context, final AuthCodeRequest request, final Uri uri, final AuthCodeListener listener) {
