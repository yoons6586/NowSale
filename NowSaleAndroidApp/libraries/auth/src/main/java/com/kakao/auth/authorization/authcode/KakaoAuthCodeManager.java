...
class KakaoAuthCodeManager implements AuthCodeManager, AuthCodeListener {
    ...
    public void requestAuthCodeWithScopes(AuthType authType, final StartActivityWrapper wrapper, List<String> scopes, AuthCodeCallback callback) {
        AuthCodeRequest request = createAuthCodeRequest(appConfig.getAppKey(), getRefreshToken(), scopes, callback);
        startTryingAuthCodeServices(authType, request, wrapper);
    }
    ...
    KakaoAuthCodeManager(final IConfiguration appConfig, final ISessionConfig sessionConfig, final AuthCodeService kakaoManager, final AuthCodeService storyManager, final AuthCodeService webManager) {
