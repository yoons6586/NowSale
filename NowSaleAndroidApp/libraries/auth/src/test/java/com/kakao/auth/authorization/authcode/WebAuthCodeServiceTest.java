...
public class WebAuthCodeServiceTest extends KakaoTestCase {
    ...
    public void createAccountLoginIntent() {
        ...
        assertTrue(intent.hasExtra(KakaoWebViewActivity.KEY_URL));
        ...
        Uri accountUri = Uri.parse(intent.getStringExtra(KakaoWebViewActivity.KEY_URL));
        Uri uri = Uri.parse(accountUri.getQueryParameter(StringSet.CONTINUE));

        assertEquals(appConfig.getAppKey(), uri.getQueryParameter(StringSet.client_id));
        assertEquals(StringSet.code, uri.getQueryParameter(StringSet.response_type));
        assertEquals(request.getRedirectURI(), uri.getQueryParameter(StringSet.redirect_uri));
    }
