...
public class DefaultAccessTokenManagerTest extends KakaoTestCase {
    ...
    public void setup() {
        super.setup();
        configuration = spy(TestAppConfig.createTestAppConfig());
        networkService = spy(new TestNetworkService());
        manager = spy(new DefaultAccessTokenManager(configuration, networkService, approvalType));
    }

    @Test
    public void createAccessTokenRequest() {
        IRequest request = new AccessTokenRequest(configuration, "auth_code", null, ApprovalType.INDIVIDUAL.toString());
        ...
        assertEquals(String.format("%s%s%s", StringSet.REDIRECT_URL_PREFIX, configuration.getAppKey(), StringSet.REDIRECT_URL_POSTFIX), params.get(StringSet.redirect_uri));

        doReturn(null).when(configuration).getClientSecret();
        AccessTokenRequest refreshRequest = new AccessTokenRequest(configuration, null, "refresh_token", ApprovalType.INDIVIDUAL.toString());
...
//        }
    ...
    public void refreshTokenWithExpiredToken() throws InterruptedException, ExecutionException {
        ...
        doAnswer(new Answer() {
            @Override
            public Future<AccessToken> answer(InvocationOnMock invocation) throws Throwable {
